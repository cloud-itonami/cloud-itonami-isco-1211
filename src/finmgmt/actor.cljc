(ns finmgmt.actor
  "FinanceManagementActor — the ISCO-08 1211 community finmgmt actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one finmgmt operation request
  ...
  The unconditional invariant: the FinanceManagementAdvisor can never
  directly commit a record the FinanceManagementGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [finmgmt.advisor :as advisor]
            [finmgmt.governor :as governor]
            [finmgmt.store :as store]))

(defn build-graph
  "Build a compiled FinanceManagementActor graph. `store` implements
  `finmgmt.store/Store`. `advisor` implements
  `finmgmt.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    ...
                                    :payload proposal}]
                       (when (= :approve-expenditure (:op proposal))
                         (store/register-spend!
                          store {:spend-id (str "spend-" (count (store/ledger store)))
